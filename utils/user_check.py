...
async def check_user(user_id):
    ...
    if time.time() > user['riot']['expiry_token']:
        ...
        await login_user_in_riot_update(user_id=user_id, cookie=cookies, access_token=access_token, expiry_token=expired_cookie, entitlements_token=entitlements_token)
        return user
    else:
        return user